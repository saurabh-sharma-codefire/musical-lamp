...
exports.login = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.REST.NOTFOUND(0, i18n.__("Account not found!"));
        }
        /* Email Verification */
        if (!user.email_verify) {
            return res.REST.SUCCESS(0, i18n.__("Account Email is not verified!"));
        }
        /* Password Validation */
        const isValidPassoword = await bcrypt.compare(password, user.password);
        if (!isValidPassoword) {
            return res.REST.SUCCESS(0, i18n.__("Invalid Credentials"));
        }
        ...
        return res.REST.SUCCESS(1, i18n.__("Logged In Successfully"), {user, token: token})
    } catch (error) {
